...
var Signature = function (options) {
    ...
    this.algo = (options.algo) ? options.algo : 'md5';
};
...
Signature.prototype.sign = function (url, options) {
    ...
    var hash = crypto.createHash(this.algo);
...
Signature.prototype.verifyUrl = function (req) {
    ...
    var hash;
    if (parsed.query.signed) {
        hash = /r:\d+;(\w+)$/g.exec(parsed.query.signed)[1];
    }

    if (!hash || !this.verifyString(url.replace(hash, ''), hash)) {
        return 'blackholed';
    }
